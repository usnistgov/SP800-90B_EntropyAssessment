...
static long double computeEntropyOfConditionedData(string inputfilename, bool iid, TestRunBase *testRun, bool quietMode) {
    ...
    if(!read_file_subset(inputfilename.c_str(), &data, 0, 0, testRun, quietMode)) {
    ...
    if (vetted && argc != 4) {
        ...
        if (!vetted) {
            if (argc == 4) {
                ...
                h_p = computeEntropyOfConditionedData(inputfilename, iid, &testRunNonIid, quietMode);
            } else {
